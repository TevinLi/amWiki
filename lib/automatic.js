...
module.exports = {
    ...
    start: function (wikis) {
        ...
        //state.libraryList = mngFolder.eachLibrary(state.libraryList);
        let libraryMD5 = {};
        ...
        this.subscriptions.add(atom.project.onDidChangePaths(function (list) {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                for (let j = 0; j < state.libraryList.length; j++) {
            ...
            for (let k = 0; k < list.length; k++) {
                if (list[k] && mngFolder.isAmWiki(list[k])) {
                    list2.push(list[k]);
                }
            }
            ...
            //MD5同步变化
            let libraryMD5 = {};
            for (let ii = 0; ii < state.libraryList.length; ii++) {
            ...
            that.treeView[0].addEventListener('drop', function (e) {
                for (let i = 0; i < e.path.length; i++) {
                    if (e.path[i].className.indexOf('project-root') >= 0) {
                        let path = e.path[i].querySelector('.header > span').getAttribute('data-path');
        ...
        if (atom.config.get('amWiki.checkAllLibraryOnAtomStart')) {
            for (let l = 0; l < state.libraryList.length; l++) {
    ...
    checkLibrary: function (state, curLibrary) {
        ...
        setTimeout(function () {
            mngFolder.readLibraryDir(curLibrary, function (err, tree, files) {
                if (err) {
                    console.warn(err);
                } else {
