...
//wiki创建器
var creator = require('../build/creator');
//文库管理
var manageWiki = require('../build/manage-wiki');
//手动刷新导航文件
var makeNav = require('../build/make-navigation');
//本地服务器模块
var localServer = require('../build/local-server');
//项目导出模块
var exportGithub = require('../build/export-github');
//自动化工具
var automatic = require('./automatic');
//页内目录创建器
var pageCatalogue = require('./page-catalogue');
//截图粘帖模块
var pasteImg = require('./paste-img');
//文件拖拽模块
var fileDrop = require('./file-drop');
...
module.exports = {
    ...
    activate: function (state) {
        ...
        if (state.libraryList) {
            for (var i = 0, item, itemPath, itemRoot; itemPath = state.libraryList[i]; i++) {
        ...
        this._subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'amWiki:create': function () {
                creator.create(function(root){
                    var wikiId = manageWiki.createWikiId(root);
                    //添加文库
