...
//文库管理
let mngWiki = require('../build/manage-wiki');
//手动刷新导航文件
...
module.exports = {
    ...
    activate: function (state) {
        ...
        this._subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'amWiki:makeNav': function () {
                let [isOnEdit, editor, editPath] = that._getEditorPath();
                if (isOnEdit || (typeof editPath !== 'undefined' && mngWiki.isAmWiki(editPath))) {
        ...
        fileDrop.listenDrop();
        window.amWikiState = {config: this._config, wikis: this._wikis};
        console.log(window.amWikiState);
    },
    //当atom即将关闭，终止所有监听
    deactivate: function () {
        localServer.destroy();
    ...
    //判断当前文库是否加入记录，未加入则补录
    _checkAddWiki: function (path) {
        if (path.indexOf('library') > 0) {
            let root = path.split('library')[0];
            let id = mngWiki.createWikiId(root);
            ...
            let id = mngWiki.createWikiId(path);
            //不包含library路径时，需要判断是否为amWiki项目
            if (typeof this._wikis[id] === 'undefined' && mngWiki.isAmWiki(path)) {
                this._wikis[id] = {
                    id: id,
                    root: path,
                    path: path + 'library/',
