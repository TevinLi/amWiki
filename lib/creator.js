...
module.exports = {
    ...
    buildAt: function (options, config, callback) {
        ...
        fs.readdir(options.outputPath, function (err, files) {
            ...
            //创建index.html
            var indexPage = fs.readFileSync(options.filesPath + 'index.tpl', 'utf-8');
            indexPage = indexPage.replace(/\{\{name\}\}/g, config.name)
                .replace('{{version}}', config.version)
                .replace('{{logo}}', config.logo);
            if (config.testing) {
                var testingTpl = fs.readFileSync(options.filesPath + 'testing.tpl', 'utf-8');
            ...
            if (!hasLibrary) {
                var home = fs.readFileSync(options.filesPath + 'home.md', 'utf-8');
            ...
            //更新导航
            updateNav.refresh(options.outputPath + 'library/');
            //添加监听
            autoNav.watcher(options.outputPath + 'library/');
