...
module.exports = {
    ...
    buildAt: function (editorPath, configPath, callback) {
        ...
        config.testing = config.testing ? '<script type="text/javascript" src="amWiki/js/amWiki-testing.js"></script>' : '';
        ...
        fs.readdir(outputPath, function (err, files) {
            ...
            var index = fs.readFileSync(filesPath + 'index.tpl', 'utf-8');
            index = index.replace(/\{\{name\}\}/g, config.name)
                .replace('{{version}}', config.version)
                .replace('{{logo}}', config.logo)
                .replace('{{amWiki-testing.js}}', config.testing);
            ...
            var fileList = [
                ...
                ['icon_home2.png', 'amWiki/images/icon_home2.png'],
