...
var makeColours = function (color) {
    ...
    if (!atBinary16 && !atRGB && !atList) {
        vscode.window.showWarningMessage('请使用标准16进制颜色、标准RGB颜色、2种特定颜色名指定颜色！本次创建使用默认颜色');
...
module.exports = {
    ...
    create: function (state) {
        ...
        //创建路径
        options.filesPath = getPackagePath().replace(/\\/g, '/')+'/files/';
        options.outputPath = options.editorPath.split('config.json')[0].replace(/\\/g, '/');
        //读取配置
        var config = fs.readFileSync(options.editorPath, 'utf-8') || '';
        if (config.length == 0) {
            vscode.window.showWarningMessage('没有读取到任何配置，继续创建么？','继续创建').then(function(msg)
            {
                if('继续创建'===msg)
                {
                    config = "{}"; 
        ...
        try {
            ...
            vscode.window.showErrorMessage('配置解析失败，请检查您的 config.json 是否有严格按 Json 格式书写！\n错误消息：' + e.message);
            parseOk = false;
        }
        if (!parseOk) {
            return;
    ...
    buildAt: function (options, config, callback) {
        ...
        fs.readdir(options.outputPath, function (err, files) {
            if (files.length > 1) {
                ...
                //     return;
                // }
            vscode.window.showWarningMessage('此处已有一些文件或文件夹，是否仍然在此创建amWiki？','继续创建').then(function(msg)
            {
                if('继续创建'!==msg)
                {
                    return;
...
function getPackagePath() {
    var extID;
    //注意，最后一个就是当前插件的ID
    var extObjs = vscode.extensions.all;
    for (var i in extObjs) {
        extID = extObjs[i].id;
    }
    var packagePath=vscode.extensions.getExtension(extID).extensionPath;
    return packagePath;
}